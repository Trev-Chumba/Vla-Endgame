...
import { GET_ASSOCIATE_PROFILE, SET_ASSOCIATE_PROFILE } from 'src/api/Endpoints';
...
const TABLE_HEAD = [
    ...
    { id: 'idNO', label: 'ID No.', alignRight: false },
    { id: 'kraPin', label: 'Kra Pin', alignRight: false },
    { id: 'mobile', label: 'mobile', alignRight: false },

    { id: 'relationship', label: 'Relationship', alignRight: false },
    { id: 'attachment', label: 'Attachments', alignRight: false },
...
export default function AssociatesProfiles({ id, updateProfileData }) {
    ...
    const [otherResidents, setOtherResidents] = useState(associates)

    const [associatesData, setAssociateData] = useState({})
    ...
    const getAssociateData = () => {
        if (id) {
            ...
            FetchApi.post(GET_ASSOCIATE_PROFILE, requestBody, (status, data) => {
                if (status) {
    ...
    const RegisterSchema = Yup.object().shape({
        name: Yup.string().required('name required').matches(stringRegExp,"Only Characters are allowed").max(50,"Too long"),
        idNO: Yup.string().max(15,"Too long"),
    ...
    const setAssociatesProfile  = (values) => {
        FetchApi.post(associatesData.associate_ID ? UPDATE_ASSOCIATE_PROFILE : SET_ASSOCIATE_PROFILE, values, (status, data) => {
            if (status) {
                // setProfileData(data)
                                    ...
                                    <CardContent>
                                        <Stack spacing={3}>
                                            ...
                                            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                                                ...
                                                {
                                                    attachments ? <Typography>{attachments[0].name}</Typography> : <Typography>{associatesData.attachments}</Typography>
                                                }
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {filteredUsers
                                            ...
                                            .map((row) => {
                                                const { subjectID, attachments, name, mobile, kraPin,
                                                    relationship, idNO, remarks } = row;
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <TableCell padding="checkbox">
                                                            ...
                                                            // onChange={(event) => handleClick(event, associate_ID)}
                                                            />
                                                        </TableCell>
                                                    ...
                                                    <TableCell align="left">{kraPin}</TableCell>
