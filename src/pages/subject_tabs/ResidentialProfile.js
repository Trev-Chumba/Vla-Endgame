...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_user) => _user.currentAddress.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
...
export default function ResidentialProfile({ id, updateProfileData }) {
  ...
  const RegisterSchema = Yup.object().shape({
    currentAddress: Yup.string().required('Current address is required').matches(stringRegExp,'Only Characters allowed'),
    ...
    postalAddress: Yup.string().matches(numericRegExp,'Only digits allowed'),
                  ...
                  <CardContent>
                    <Stack spacing={3}>
                      ...
                      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                        ...
                        {
                          attachments ? <Typography>{attachments[0].name}</Typography> : <Typography>{residentialData.attachments}</Typography>
                        }
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers
                    ...
                    .map((row) => {
                      const { subjectID, attachments, coordinates, currentAddress, otherAddress,
                        physicalAddress, postalAddress, remarks,res_INFO_ID } = row;;
                        ...
                        >
                          <TableCell padding="checkbox">
                            ...
                            <TableCell align="right">
                              <Button
                                onClick={() => setResidentialData(row)}>
